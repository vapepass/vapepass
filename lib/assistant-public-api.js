const API_URL = (process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000/api/v1').replace(/\/+$/, '');
/** Chat turns may include OpenAI ranking; allow cold starts + model latency. */
const REQUEST_TIMEOUT_MS = 60000;

/** When the chat UI runs inside the merchant embed iframe, set the host page origin. */
let embedParentOrigin = null;

export function setEmbedParentOrigin(origin) {
  embedParentOrigin = origin && String(origin).trim() ? String(origin).trim() : null;
}

async function publicRequest(path, options = {}) {
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), REQUEST_TIMEOUT_MS);

  try {
    const headers = { 'Content-Type': 'application/json', Accept: 'application/json' };
    if (embedParentOrigin) {
      headers['X-Vapepass-Parent-Origin'] = embedParentOrigin;
    }

    const response = await fetch(`${API_URL}${path}`, {
      method: options.method || 'GET',
      headers,
      body: options.body ? JSON.stringify(options.body) : undefined,
      signal: controller.signal,
    });

    const data = await response.json().catch(() => ({}));
    if (!response.ok || data.success === false) {
      throw new Error(data.message || 'Request failed');
    }
    return data;
  } catch (error) {
    if (error?.name === 'AbortError') {
      throw new Error(
        'The assistant is taking too long to respond. The API may be waking up — please try again in a few seconds.'
      );
    }
    // Browser "Failed to fetch" = network / CORS / API unreachable
    if (
      error?.name === 'TypeError' ||
      /failed to fetch|networkerror|load failed/i.test(String(error?.message || ''))
    ) {
      throw new Error(
        'Could not reach the VapePass API. Make sure the backend is running, then try again.'
      );
    }
    throw error;
  } finally {
    clearTimeout(timer);
  }
}

export async function getAssistantWidgetConfig(storeId) {
  const res = await publicRequest(`/assistant/widget/${storeId}`);
  return res.data.config;
}

export async function startAssistantSession(storeId, sessionKey) {
  const res = await publicRequest('/assistant/session', {
    method: 'POST',
    body: { storeId, sessionKey },
  });
  return res.data.session;
}

export async function sendAssistantMessage(storeId, sessionKey, message) {
  const res = await publicRequest('/assistant/chat', {
    method: 'POST',
    body: { storeId, sessionKey, message },
  });
  return res.data.session;
}
