...
export function resolveOptionIcon(option) {
  ...
  if (/\b(fruit|berry|citrus|tropical|mango|grape|melon|exotic)\b/.test(label)) return Grape;
  if (/\b(sweet|candy|dessert|creamy|cream)\b/.test(label)) return Candy;
  if (/\b(beverage|coffee|drink)\b/.test(label)) return CupSoda;
  if (/\b(tobacco|rich)\b/.test(label)) return Coffee;
  if (/\b(disposable|puff)\b/.test(label)) return Zap;
  if (/\b(device|kit|mod|hardware)\b/.test(label)) return Store;
  if (/\b(pod|cartridge)\b/.test(label)) return CircleDot;
  if (/\b(coil|tank|batter|charger|accessor)\b/.test(label)) return Settings;
  if (/\b(pouch)\b/.test(label)) return ShoppingCart;
  if (/\b(e-?liquid|juice|liquid)\b/.test(label)) return Droplets;
