...
export async function getAdminSetupRequests(params = {}) {
  const query = new URLSearchParams();
  if (params.page) query.set('page', String(params.page));
  if (params.limit) query.set('limit', String(params.limit));
  if (params.status) query.set('status', params.status);
  const qs = query.toString();
  const res = await apiRequest(`/admin/setup-requests${qs ? `?${qs}` : ''}`);
  return res.data;
}

export async function updateSetupRequestStatus(requestId, status) {
  const res = await apiRequest(`/admin/setup-requests/${requestId}/status`, {
    method: 'PATCH',
    body: { status },
  });
  return res.data;
